...
/**
 * Feature: pyxis-course-materials, Property 5: Access list add is idempotent
 *
 * Validates: Requirements 2.6
 *
 * For any valid Kerberos identifier, adding it to the access list twice should
 * result in the same access list state as adding it once (same size, same membership).
 */
...
describe('Property 5: Access list add is idempotent', () => {
  beforeEach(() => {
  ...
  it('adding the same kerberos twice yields the same state as adding once', async () => {
    await fc.assert(
      fc.asyncProperty(validKerberosArb, async (kerberos) => {
        ...
        const sizeAfterFirstAdd = accessListStore.size;

        // Add again (same kerberos)
        await addToAccessList(kerberos);
        const sizeAfterSecondAdd = accessListStore.size;

        // Store size should not change after second add (idempotence)
        expect(sizeAfterSecondAdd).toBe(sizeAfterFirstAdd);

        // Membership should still return true
